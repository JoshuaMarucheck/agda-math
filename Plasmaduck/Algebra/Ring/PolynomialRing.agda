...
module RawPolynomials (ring : Ring c ℓ) where
    ...
    Polynomial : Set (c ⊔ ℓ)
    Polynomial = Σ (ℕ → Carrier) λ p → Σ ℕ λ n → ∀ {m} → .(n ≤ m) → IsZero (p m)

    get : (p : Polynomial) → ℕ → Carrier
    get p n = p .proj₁ n

    HasDegree : Polynomial → ℕ → Set ℓ
    HasDegree p n = IsNonzero (get p n) × (∀ {m} → .(n < m) → IsZero (get p m))
